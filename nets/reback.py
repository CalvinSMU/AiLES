...
class _Res34_unet(nn.Module):
    def __init__(self, num_classes=1, num_channels=1):
        ...
        self.finalconv1 = nn.Sequential(nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True),
...
class nonLocal(nn.Module):
    def __init__(self, in_channels, n_filters):
        ...
        self.conv1 = nn.Sequential(nn.Conv2d(64, in_channels, 8, padding=0, stride=8, bias=False),
                                   nn.BatchNorm2d(in_channels), nn.ReLU())
        # self.bn1 = nonlinearity
        self.conv_fai = nn.Sequential(nn.Conv2d(in_channels, n_filters, 1, padding=0, stride=1, bias=False),
                                      nn.BatchNorm2d(n_filters), nn.ReLU())
        self.conv_sit = nn.Sequential(nn.Conv2d(in_channels, n_filters, 1, padding=0, stride=1, bias=False),
                                      nn.BatchNorm2d(n_filters), nn.ReLU())
        self.conv_gama = nn.Sequential(nn.Conv2d(in_channels, n_filters, 1, padding=0, stride=1, bias=False),
                                       nn.BatchNorm2d(n_filters), nn.ReLU())
...
class _nonlocal_unet(nn.Module):
    def __init__(self, num_classes=1, num_channels=1):
        ...
        self.finalconv1 = nn.Sequential(nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True),
                                        nn.Conv2d(64 + 16, 32, 3, padding=1))
        self.finalconv1.apply(weights_init_kaiming)
        self.finalrelu1 = nonlinearity
        self.finalconv2 = nn.Conv2d(32, 32, 3, padding=1)
        self.finalconv2.apply(weights_init_kaiming)
...
# 将res map传输到decoder，个人觉得直接传输到encoder会出问题，encoder的职责就是提取特征，至于挑选那是decoder应该做的事情。
...
class _Reback_v2(nn.Module):
    def __init__(self, num_classes=1, num_channels=1):
        ...
        resnet.load_state_dict(torch.load('./pretrain/resnet34-333f7ec4.pth'))
        # self.firstconv = resnet.conv1   # 采用resnet34的首层，代替原文适用于超声图像的卷积层
        self.firstconv = nn.Conv2d(3, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)
        self.firstconv.weight = torch.nn.Parameter(resnet.conv1.weight[:, :3, :, :])
        self.firstbn = resnet.bn1
        self.firstrelu = resnet.relu
        self.firstmaxpool = resnet.maxpool
        self.encoder1 = resnet.layer1
