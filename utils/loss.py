...
class FocalLoss(nn.Module):
    ...
    def focalLoss(self, y_true, y_pred, weight, gamma=1):
        ...
        if self.batch:
            ...
            loss_value = -y_true * (1 - y_pred) ** gamma * torch.log(y_pred) - (
...
class iou_loss2(nn.Module):
    ...
    def focalLoss(self, y_true, y_pred, gamma=1):
        ...
        if self.batch:
            y_pred = y_pred.contiguous().view(-1, y_pred.size(2))
            y_true = y_true.contiguous().view(-1, y_true.size(2))
            loss_value = -y_true * (1 - y_pred) ^ gamma * torch.log(y_pred) - (1 - y_true) * y_pred ^ gamma * torch.log(
                1 - y_pred)
        else:
    ...
    def soft_iou_coeff(self, y_true, y_pred):
        ...
        score = (intersection + smooth) / (i + j + smooth - intersection) * self.weight + (1 - self.weight) * (
                    intersection2 + smooth) / (i_ + j_ + smooth - intersection2)
        return score.mean()
...
class dice_bce_loss2(nn.Module):
    ...
    def soft_dice_coeff(self, y_true, y_pred):
        ...
        score = weight * (2. * intersection + smooth) / (i + j + smooth) + (1 - weight) * (
                    2. * intersection2 + smooth) / (i2 + j2 + smooth)
        return score.mean()
